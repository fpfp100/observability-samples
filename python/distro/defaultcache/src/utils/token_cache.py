...
"""
Token caching utilities for Kairo exporter authentication.
"""
...
# Global token cache for Kairo exporter
_agentic_token_cache = {}


def cache_agentic_token(tenant_id: str, agent_id: str, token: str) -> None:
    """Cache the agentic token for use by Kairo exporter."""
...
def get_cached_agentic_token(tenant_id: str, agent_id: str) -> str | None:
    """Retrieve cached agentic token for Kairo exporter."""
    key = f"{tenant_id}:{agent_id}"
