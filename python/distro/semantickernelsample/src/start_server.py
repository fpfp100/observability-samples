...
def start_server(agent_application: AgentApplication, auth_configuration: AgentAuthConfiguration):
    ...
    # Configure Microsoft OpenTelemetry with Agent365 (A365) trace export.
    # Disables Azure Monitor and enables the A365 exporter; token_resolver
    # authenticates exports using the agent's cached observability token.
    ...
    # Enable Semantic Kernel auto-instrumentation via the distro's instrumentor.
    # This automatically creates inference spans for all SK kernel invocations.
    from microsoft.opentelemetry._semantic_kernel import SemanticKernelInstrumentor
    SemanticKernelInstrumentor().instrument()

    # Configure observability hosting middleware
