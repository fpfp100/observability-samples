...
# TOKEN_MODE=obo: uses adapter pipeline with agentic auth (requires dev tunnel)
...
def start_server(agent_application: AgentApplication, auth_configuration: AgentAuthConfiguration):
    token_mode = environ.get("TOKEN_MODE", "s2s").lower()
    use_s2s = token_mode == "s2s"

    if use_s2s:
        # S2S: bypass adapter (it calls get_agentic_instance_token which fails with emulator)
        ...
        # OBO: use adapter pipeline with agentic auth (requires dev tunnel)
    ...
    if not use_s2s and agent_application:
    ...
    use_microsoft_opentelemetry(
        ...
        a365_use_s2s_endpoint=use_s2s,  # OBO → /observability, S2S → /observabilityService
        a365_enable_observability_exporter=True,
    )

    logger.info(f"Observability configured: mode={token_mode}, s2s_endpoint={use_s2s}")

    port = int(environ.get("PORT", "3987"))
